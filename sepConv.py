...
def cp_kernel(strFunction, objectVariables):
...
def cp_launch(strFunction, strKernel):
    return cp.cuda.compile_with_cache(strKernel).get_function(strFunction)


# end

class SepConv(torch.autograd.Function):
    def __init__(self):
    ...
    def forward(ctx, input, vertical, horizontal):
        ...
        if input.is_cuda == True:
            ...
            n = output.nelement() # = intSample*intInputDepth*intOutputHeight*intOutputWidth
            cp_launch('kernel_Sepconv_updateOutput', cp_kernel('kernel_Sepconv_updateOutput', {
                'input': input,
                'vertical': vertical,
    ...
    def backward(ctx, gradOutput):
        ...
        if input.is_cuda == True:
            ...
            cp_launch('kernel_SeparableConvolution_updateGradVertical', cp_kernel('kernel_SeparableConvolution_updateGradVertical', {
                'gradLoss': gradOutput,
                'input': input,
                'horizontal': horizontal,
            ...
            cp_launch('kernel_SeparableConvolution_updateGradHorizontal', cp_kernel('kernel_SeparableConvolution_updateGradHorizontal', {
                'gradLoss': gradOutput,
                'input': input,
                'vertical': vertical,
