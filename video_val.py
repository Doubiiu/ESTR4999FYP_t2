...
class TestVideoVal:
    def __init__(self, file_list):
        self.transform = transforms.Compose([
            transforms.Resize((1088, 1920)), # (h, w)
            transforms.ToTensor()
            ])
        with open(file_list, 'r') as f:
            lines = [line.rstrip('\n') for line in f.readlines()]
        self.triplet_file_list = ['data/video_frames/'+f for f in lines]
    ...
    def test_image(self, model, output_dir = 'data/video_frames'):
        sum_psnr = 0
        sum_ssim = 0
        ...
        for i in range(len(self.triplet_file_list)):
            ...
            torchvision.utils.save_image(frame_i, self.triplet_file_list[i] + '/imi.png', range=(0, 1))
